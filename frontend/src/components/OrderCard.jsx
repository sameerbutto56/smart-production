...
const OrderCard = ({ order, onUpdateStage, userRole, isUnseen = false, onMarkSeen, selected, onToggleSelect }) => {
  ...
  const handleVerificationToggle = useCallback(async (idx) => {
    const key = String(idx);
    const newVal = !(productVerification[key] === true);
    // Optimistic update
    setProductVerification(prev => ({ ...prev, [key]: newVal }));
    try {
      await api.patch(`/api/orders/${order.id}/toggle-product-verification`, { productIndex: idx });
      toast.success(newVal ? `Product #${idx + 1} Verified ✓` : `Product #${idx + 1} Unverified`);
    } catch (err) {
      console.error(err);
              ...
              <div key={idx} className={`${isMultiProd || items.length > 1 ? 'bg-gray-900/40 p-3 rounded-xl border border-gray-800/70' : ''}`}>
                ...
                {(isMultiProd || sortedItems.length > 1) && (
                  <div className="flex items-center gap-2 mb-3 pb-2 border-b border-gray-800/50">
                    ...
                    <span className="text-xs font-black text-white uppercase">{isUrdu ? toUrduName(p.productType || p.name || `Item ${idx + 1}`) : (p.productType || p.name || `Item ${idx + 1}`)}</span>
                    {p.color && <span className="text-[9px] text-gray-500">({isUrdu ? toUrduName(p.color) : p.color})</span>}
                    {isNotAvail && (
                    ...
                    return (
                      <div key={idx} className="border border-gray-800 rounded-xl p-2.5 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-cyan-400">#{idx + 1} {isUrdu ? toUrduName(p.productType || p.name || 'Product') : (p.productType || p.name || 'Product')}</span>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-10 custom-scrollbar">
                                ...
                                <tr className="border-b border-gray-800/50 transition-colors hover:bg-gray-900/30">
                                  <td className="py-4 px-4 text-gray-500 font-black">{idx + 1}</td>
                                   <td className="py-4 px-4 font-bold uppercase">
                                     ...
                                     <span className="text-white">{isUrdu ? toUrduName(p.productType || p.name) : (p.productType || p.name) || '—'}</span>
                                   </td>
              ...
              {/* 02. Measurements */}
              {(() => {
                // Build per-item measurement arrays
                const allItemsLocal = isMultiItem ? orderItems : null;
                const measLabels = {
                  'Shirt Length': isUrdu ? 'قمیض لمبائی' : 'Shirt Length',
                  'Shoulder': isUrdu ? 'کندھا' : 'Shoulder',
                  'Sleeves Length': isUrdu ? 'آستین لمبائی' : 'Sleeves Length',
                  'Sleeves Hole': isUrdu ? 'آستین سوراخ' : 'Sleeves Hole',
                  'Chest': isUrdu ? 'سینہ' : 'Chest',
                  'Bottom': isUrdu ? 'باٹم' : 'Bottom',
                  'Waist': isUrdu ? 'کمر' : 'Waist',
                  'Length': isUrdu ? 'لمبائی' : 'Length',
                  'Pancha': isUrdu ? 'پنچہ' : 'Pancha',
                  'Thighs': isUrdu ? 'ران' : 'Thighs',
                  'Asan': isUrdu ? 'آسن' : 'Asan',
                };
                ...
                if (allItemsLocal) {
                  // Multi-item: check each item for measurement data
                  const itemsWithMeas = allItemsLocal.map((item, idx) => {
                    const p = item.productDetails || {};
                          ...
                          <div key={idx} className="bg-teal-500/5 border border-teal-500/20 rounded-2xl p-4">
                            ...
                            {rows.length > 0 && (
                              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {rows.map((r, ri) => (
                                  <div key={ri} className="flex justify-between items-center bg-gray-900/50 px-3 py-2 rounded-xl border border-gray-800/50">
                                    <span className="text-[10px] font-black text-gray-500 uppercase tracking-wider">{r.label}</span>
                                    <span className="text-sm font-black text-white ml-2">{r.value}</span>
                                  </div>
                                ))}
                              </div>
                            )}
