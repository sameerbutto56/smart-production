...
export function printJobSheet(order, userRole, lang = 'ur', sections = {}) {
  ...
  win.document.write(`<p style="font-size:20px;color:#000;margin-top:2px;font-weight:700">${isUrdu ? 'آرڈر #' : 'Order #'}<span style="font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600">${order.orderNumber || order.id?.slice(0, 8)}</span></p>`);
  ...
  win.document.write(`<p style="font-size:15px;color:#000;font-weight:500;margin-top:2px">${isUrdu ? 'تیار کردہ:' : 'Generated:'} <span style="font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600">${generatedDate} ${generatedTime}</span></p>`);
  win.document.write(`</div>`);
  ...
  win.document.write(`<div><span style="font-size:16px;font-weight:700;color:#000">${isUrdu ? 'اندراج کا وقت:' : 'Entry Time:'}</span> <span style="font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600;color:#000">${entryTime}</span></div>`);
  if (shopifyDate) {
  ...
  if (isMultiItem) {
    ...
    allItems.forEach((item, idx) => {
      ...
      win.document.write(`<td class="product-name">${verified ? '<span style="display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;border-radius:0;background:#fff;color:#000;border:1.2px solid #000;font-size:10px;margin-right:4px" title="Verified">✓</span>' : ''}${pu(p.productType || p.name) || '—'}</td>`);
    ...
    if (hasAnyCustomization || outHasEngraving) {
      ...
      if (hasAnyCustomization) {
        brandingItems.forEach((item, idx) => {
          ...
          if (c?.engravingType) {
            ...
            win.document.write(`<p style="${isUrdu ? 'font-size:18px;font-weight:700' : 'font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600'};text-transform:uppercase;color:#000;margin-bottom:2px"${isUrdu ? ' class="urdu"' : ''}>${sec.engravingType}: ${engravingLabel}</p>`);
            win.document.write(`</div>`);
          }
      ...
      if (outHasEngraving) {
        ...
        if (order.engravingType) {
          ...
          win.document.write(`<p style="${isUrdu ? 'font-size:20px;font-weight:700' : 'font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600'};color:#000;margin-bottom:4px"${isUrdu ? ' class="urdu"' : ''}>${typeLabel}: ${etLabel}</p>`);
        }
        if (order.engravingText) {
          win.document.write(`<p style="${isUrdu ? 'font-size:20px;font-weight:700' : 'font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600'};color:#000;margin-bottom:4px"${isUrdu ? ' class="urdu"' : ''}>${sec.engravingType}: ${order.engravingText}</p>`);
        }
...
export function printDispatchSheet(order, lang = 'ur') {
  ...
  win.document.write(`<h2 style="font-size:20px;font-weight:700;text-transform:uppercase;color:#000;letter-spacing:0"${isUrdu ? ' class="urdu"' : ''}>${isUrdu ? 'آرڈر نمبر' : 'Order #'}<span style="font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600">${order.orderNumber || order.id?.slice(0, 8)}</span></h2>`);
  win.document.write(`</div>`);
  ...
  // ─── PRODUCTS TABLE ───
  win.document.write(`<div class="section-title${isUrdu ? ' urdu' : ''}" style="margin-top:4px">${isUrdu ? 'آرٹیکلز' : 'Products'}</div>`);
  const rawPd = parseJSON(order.productDetails);
  const allItems = Array.isArray(rawPd) ? rawPd : null;
  ...
  if (isMultiItem) {
    ...
    allItems.forEach((item, idx) => {
      ...
      win.document.write(`<tr>`);
      win.document.write(`<td style="font-weight:700">${idx + 1}</td>`);
      win.document.write(`<td class="product-name">${pu(p.productType || p.name) || '—'}</td>`);
  ...
  // ─── FINANCIAL SUMMARY ───
  win.document.write(`<div class="section-title${isUrdu ? ' urdu' : ''}" style="margin-top:8px">${isUrdu ? 'مالی خلاصہ' : 'Financial Summary'}</div>`);
  ...
  if (order.paymentStatus === 'PAID' || order.paymentStatus === 'FULL_PAID') {
    ...
    win.document.write(`<span style="color:#000">${isUrdu ? 'ایڈوانس موصول:' : 'Advance Received:'} <span style="font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600">${currency(order.advanceAmount)}</span></span>`);
    win.document.write(`<span style="color:#000">${isUrdu ? 'باقی:' : 'Remaining:'} <span style="font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600">${currency(parseFloat(order.totalPrice) - parseFloat(order.advanceAmount || 0))}</span></span>`);
    win.document.write(`</div>`);
  } else {
  ...
  if (order.deliveryMethod) {
    ...
    if (order.trackingNumber) {
      win.document.write(`<p style="font-family:Inter,Arial,sans-serif;font-size:15px;font-weight:600;color:#000;margin-top:4px">${isUrdu ? 'ٹریکنگ:' : 'Tracking:'} ${order.trackingNumber}</p>`);
    }
    win.document.write(`</div>`);
  }
