...
export const exportDailyDepositsToExcel = (requirements = [], outlet = 'Outlet') => {
  const rows = requirements.map(r => ({
    ...
    'Base Required (Available Cash)': fmt(r.baseRequired ?? r.availableCash ?? r.requiredAmount),
    'Previous Balance': r.previousPending > 0 ? `+${fmt(r.previousPending)} Short` : r.previousExcess > 0 ? `-${fmt(r.previousExcess)} Credit` : '0',
    'Net Required': fmt(r.netRequired ?? r.requiredAmount),
    'Actual Deposit': fmt(r.depositedAmount),
    'Applied to Prev Pending': fmt(r.appliedToPrev || 0),
    'Applied to Current Day': fmt(r.appliedToCurrent || r.depositedAmount),
    'Remaining Pending': fmt(r.pendingAmount),
    'Excess Credit': fmt(r.excessAmount || 0),
    'Status': r.status,
