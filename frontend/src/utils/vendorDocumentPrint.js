...
// Implements:
// 1. Picture 1: Pre-printed letterhead branding (compact header + footer with geometric shapes and contact details)
// 2. Picture 2: Microsoft Word document layout (centered title, subheaders, blue callout box, navy table, totals, amount in words, dual signatures)
// 3. Compact header & footer footprint to leave maximum vertical room for body items.
...
// Letterhead Header (Picture 1 style, compact ~46px height)
function buildHeaderHTML(logoUrl) {
  return `
  <div class="a4-header" style="margin-bottom: 6px;">
    <div style="display: flex; align-items: center; justify-content: flex-start; gap: 8px;">
      <img src="${logoUrl}" alt="ENAMELS" style="height: 34px; width: auto; object-fit: contain;">
      <div>
        <div style="font-size: 16px; font-weight: 900; letter-spacing: 1.5px; color: #433285; line-height: 1; font-family: 'Segoe UI', Arial, sans-serif;">ENAMELS</div>
        <div style="font-size: 7px; font-weight: 700; letter-spacing: 1.8px; color: #b8860b; text-transform: uppercase; margin-top: 2px;">FASHION | PROFESSION | SCRUBS</div>
      </div>
    </div>
    <div style="display: flex; align-items: center; width: 100%; margin-top: 4px;">
      <div style="height: 3px; width: 150px; background: #4d3a86; border-radius: 1px;"></div>
      <div style="height: 1.5px; flex: 1; background: #c59b27;"></div>
    </div>
  </div>`;
}

// Letterhead Footer (Picture 1 style, compact ~48px height)
// Left: Geometric purple + gold triangles with diagonal accent stripes
// Right: Contact info with WhatsApp, Facebook, Instagram, Email, Web
function buildFooterHTML() {
  return `
  <div class="a4-footer" style="margin-top: 8px; padding-top: 4px; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: flex-end;">
    <!-- Left Graphic (Picture 1 Geometric Shapes) -->
    <div style="display: flex; align-items: flex-end;">
      <svg viewBox="0 0 160 55" width="145" height="48" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: block;">
        <!-- Diagonal accent stripes -->
        <line x1="12" y1="46" x2="38" y2="20" stroke="#584496" stroke-width="2.5" stroke-linecap="round" />
        <line x1="8" y1="50" x2="34" y2="24" stroke="#C59B27" stroke-width="2" stroke-linecap="round" />
        <line x1="4" y1="54" x2="28" y2="30" stroke="#584496" stroke-width="2" stroke-linecap="round" />
        
        <!-- Gold triangle -->
        <polygon points="0,55 38,55 18,34" fill="#C59B27" />
        
        <!-- Large Purple triangle -->
        <polygon points="32,55 125,55 78,10" fill="#584496" />
      </svg>
    </div>

    <!-- Right Contact Info (Picture 1 metadata with icons) -->
    <div style="font-size: 8px; font-weight: 700; color: #475569; line-height: 1.35; text-align: left; padding-bottom: 2px;">
      <div style="display: flex; align-items: center; gap: 5px; margin-bottom: 1.5px;">
      ...
      <div style="display: flex; align-items: center; gap: 5px; margin-bottom: 1.5px;">
        ...
        <span>info@enamelsonline.com</span>
      </div>
      <div style="display: flex; align-items: center; gap: 5px;">
        ...
        <span>www.enamelsonline.com</span>
      </div>
    </div>
  </div>`;
}

// Builds the full A4 document HTML matching Picture 2 (Body layout) and Picture 1 (Header + Footer)
export function generateDocumentHTML(order, kind, logoUrl) {
  ...
  (order.items || []).forEach((it, idx) => {
    ...
    itemsRows += `
      <tr style="border-bottom: 1px solid #e2e8f0;">
        <td style="padding: 4px 6px; text-align: center; border: 1px solid #cbd5e1; font-weight: 600; color: #64748b; font-size: 9px;">
        ...
        <td style="padding: 4px 6px; text-align: center; border: 1px solid #cbd5e1; font-weight: 700; color: #0f172a; font-size: 9.5px;">
          ${it.quantity || 1}
        </td>
        <td style="padding: 4px 8px; text-align: right; border: 1px solid #cbd5e1; color: #334155; font-size: 9.5px;">
          Rs. ${parseFloat(it.unitPrice || 0).toLocaleString()}
        </td>
        <td style="padding: 4px 8px; text-align: right; border: 1px solid #cbd5e1; font-weight: 700; color: #0f172a; font-size: 9.5px;">
  ...
  <div class="a4-container">
    ...
    <!-- IN-BETWEEN DATA (Picture 2) -->
    <div class="a4-body">
      <!-- Centered Document Title -->
      <div style="text-align: center; margin-bottom: 5px;">
        <span style="font-size: 15px; font-weight: 900; letter-spacing: 2px; color: #0f172a; text-transform: uppercase; border-bottom: 1.5px solid #0f172a; padding-bottom: 1px;">
          ${isInvoice ? 'INVOICE' : 'QUOTATION'}
        </span>
      </div>

      <!-- Top Subheader: Company Info (Left) & Ref/Date (Right) -->
      ...
      <!-- Blue Shaded Callout Box (Picture 2) -->
      ...
      <!-- Main Items Table (Picture 2 Dark Navy Header) -->
      ...
      <div style="display: flex; justify-content: flex-end; margin-bottom: 6px;">
        <table style="border-collapse: collapse; font-size: 8.5px; width: 250px; background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 4px;">
          <tr>
            <td style="padding: 2.5px 8px; font-weight: 700; color: #334155; border-bottom: 1px solid #dbeafe;">Net Total / (PKR):</td>
            <td style="padding: 2.5px 8px; text-align: right; font-weight: 700; color: #0f172a; border-bottom: 1px solid #dbeafe;">Rs. ${parseFloat(order.totalOrderValue || 0).toLocaleString()}</td>
          </tr>
          ${parseFloat(order.deliveryCharges || 0) > 0 ? `
          <tr>
            <td style="padding: 2.5px 8px; font-weight: 700; color: #334155; border-bottom: 1px solid #dbeafe;">Delivery Charges / (PKR):</td>
            <td style="padding: 2.5px 8px; text-align: right; font-weight: 700; color: #0f172a; border-bottom: 1px solid #dbeafe;">Rs. ${parseFloat(order.deliveryCharges).toLocaleString()}</td>
          </tr>` : ''}
          ${parseFloat(order.discount || 0) > 0 ? `
          <tr>
            <td style="padding: 2.5px 8px; font-weight: 700; color: #16a34a; border-bottom: 1px solid #dbeafe;">Discount / (PKR):</td>
            <td style="padding: 2.5px 8px; text-align: right; font-weight: 700; color: #16a34a; border-bottom: 1px solid #dbeafe;">-Rs. ${parseFloat(order.discount).toLocaleString()}</td>
          </tr>` : ''}
          <tr style="background: #dbeafe;">
            <td style="padding: 3px 8px; font-weight: 900; color: #1e3a8a; border-bottom: 1px solid #bfdbfe;">Grand Total / (PKR):</td>
            <td style="padding: 3px 8px; text-align: right; font-weight: 900; font-size: 10px; color: #1e3a8a; border-bottom: 1px solid #bfdbfe;">Rs. ${parseFloat(order.grandTotal || 0).toLocaleString()}</td>
          </tr>
          ${totalPaid > 0 ? `
          <tr>
            <td style="padding: 2.5px 8px; font-weight: 700; color: #334155; border-bottom: 1px solid #dbeafe;">Paid Amount / (PKR):</td>
            <td style="padding: 2.5px 8px; text-align: right; font-weight: 700; color: #0f172a; border-bottom: 1px solid #dbeafe;">Rs. ${totalPaid.toLocaleString()}</td>
          </tr>` : ''}
          ${isInvoice || remaining > 0 ? `
          <tr>
            <td style="padding: 2.5px 8px; font-weight: 900; color: ${remaining > 0.01 ? '#b91c1c' : '#15803d'};">Remaining Balance / (PKR):</td>
            <td style="padding: 2.5px 8px; text-align: right; font-weight: 900; color: ${remaining > 0.01 ? '#b91c1c' : '#15803d'};">Rs. ${remaining.toLocaleString()}</td>
          </tr>` : ''}
        </table>
      </div>

      <!-- Amount in Words (Picture 2) -->
      <div style="text-align: center; margin: 6px 0 14px 0; font-size: 8.5px; font-weight: 700; font-style: italic; color: #1e293b; letter-spacing: 0.3px;">
        Amount in Words : <span style="text-decoration: underline;">${numberToWords(order.grandTotal || 0)} ONLY.</span>
      </div>

      <!-- Notes if any -->
      ${order.notes ? `
      <div style="background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 4px; padding: 4px 8px; margin-bottom: 10px; font-size: 8px; color: #475569;">
        <strong>Notes:</strong> ${order.notes}
      </div>` : ''}

      <!-- Dual Signatures (Picture 2) -->
      <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-top: 14px; margin-bottom: 6px; padding: 0 16px;">
        <div style="text-align: center;">
          <div style="width: 140px; border-top: 1.5px solid #0f172a; margin-bottom: 3px;"></div>
    ...
    <!-- BOTTOM LETTERHEAD FOOTER (Picture 1) -->
    ${buildFooterHTML()}
  </div>
  `;
}
