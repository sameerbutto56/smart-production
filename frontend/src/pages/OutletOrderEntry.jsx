...
const OutletOrderEntry = () => {
  ...
  const handleLookup = useCallback(async () => {
    ...
    try {
      ...
      if (clients.length === 1) {
        ...
        setShowResults(true);
        if (clients.length === 0) {
          // Auto-generate order number when searched by order number
          if (isOrderNumberSearch) {
            try {
              const genRes = await api.get('/api/outlet-orders/generate-number');
              setAutoGeneratedOrderNumber(genRes.data.orderNumber);
      ...
      if (err.response?.status === 404) {
        setLookedUp(true);
        setClientData(null);
        // Auto-generate order number if searched by order number format
        if (/^[A-Za-z]{2,4}-\d+$/.test(searchQuery.trim())) {
          try {
            const genRes = await api.get('/api/outlet-orders/generate-number');
            setAutoGeneratedOrderNumber(genRes.data.orderNumber);
          } catch { setAutoGeneratedOrderNumber(''); }
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const orderNumberToUse = autoGeneratedOrderNumber || (manualOrderNumber && manualOrderNumber.trim()) || undefined;
      const payload = {
              ...
              <div className="bg-amber-900/20 border border-amber-700 rounded-xl p-3">
                ...
                {autoGeneratedOrderNumber ? (
                  <div className="flex items-center gap-3">
                    <span className="text-lg font-black text-white tracking-wider">{autoGeneratedOrderNumber}</span>
                    <span className="text-[10px] font-bold text-gray-500 bg-gray-800 px-2 py-1 rounded-lg">Auto-generated</span>
                  </div>
                ) : (
