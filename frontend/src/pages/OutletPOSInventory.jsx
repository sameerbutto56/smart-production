...
/* ─── MANAGEMENT: full CRUD with outlet tabs (STORE / ADMIN / SUPER_ADMIN) ─── */
const ManagementInventory = () => {
  const { user } = useAuth();
  const defaultOutlet = 'Johar Town';
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      if (!resolvedCategory) { toast.error('Please specify a category'); setEditSubmitting(false); return; }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.selectedProductId) { toast.error('Please select a store product'); return; }
    setSubmitting(true);
    try {
      const validVariants = formData.variants.filter(v => v.color || v.size || v.stock > 0 || v.price > 0);
      if (validVariants.length === 0) { toast.error('At least one variant is required'); setSubmitting(false); return; }
  ...
  const openInitModal = useCallback(async () => {
    ...
    try {
      ...
      for (const item of storeItems) {
        ...
        data[item.id] = {
          name: item.name, category: item.category, fabric: item.fabric || '', imageUrl: item.imageUrl || '',
      ...
      setInitData(data);
    } catch { toast.error('Failed to load store products for initialization'); }
          ...
          for (const outlet of ALL_OUTLETS) {
            ...
            if (stock > 0) {
              stockData[outlet].push({
                sourceItemId, name: product.name, category: product.category,
                color: v.color || null, size: v.size || null,
                fabric: product.fabric || null, price: null, stock,
                imageUrl: product.imageUrl || null, variants: product.variants || null
              });
            }
          }
        }
    ...
    <div className="space-y-6 pb-20 px-4">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-bold text-gray-400">Manage all outlet inventories</p>
        </div>
