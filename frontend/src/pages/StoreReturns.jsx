...
import { Package, RotateCcw, RefreshCw, CheckCircle, XCircle, Factory, Eye, Box, Undo2, Truck } from 'lucide-react';
...
const StoreReturns = ({ refreshKey }) => {
  ...
  const fetchCases = useCallback(async () => {
    setLoading(true);
    try {
      // RETURN cases sit with the Store; REPLACEMENT cases arrive after Faisal approval
      ...
      const repList = (repRes.data.cases || []).filter(c => c.routedTo === 'STORE');
  ...
  const Section = ({ title, icon, color, list, kind, emptyText }) => (
          ...
          <div key={c.id} className="theme-bg-subtle border-2 theme-border rounded-2xl p-4 md:p-5">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                ...
                {kind === 'replacement' && c.specialNote && (
            ...
            {expandedId === c.id && (
              <div className="mt-4 space-y-3">
                <div>
                  <p className="text-[10px] font-black theme-text-muted uppercase mb-2">{kind === 'return' ? 'Returned Goods (to restock)' : 'Replacement Items (to deliver)'}</p>
                  <div className="space-y-1.5">
                    {parseItems(kind === 'return' ? c.originalProducts : c.replacementItems).map((item, i) => {
                      ...
                      return (
                        <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl px-3 py-2 text-xs flex items-center justify-between">
                          <div>
                            <p className="font-bold text-white">{pd.name || pd.productType || item.name || 'Product'}</p>
                            <p className="text-gray-500 text-[10px]">{(pd.color || item.color || '')} {(pd.size || item.size || '')} × {item.quantity || 1}</p>
                          </div>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {kind === 'return' ? (
                    ...
                    <>
                      <button onClick={() => runDeduct(c)} disabled={processingId === c.id} className="bg-amber-600 hover:bg-amber-500 text-white font-black py-2.5 px-4 rounded-xl text-xs flex items-center gap-1 disabled:opacity-50">
                        <Truck size={14} /> Deduct Stock & Ready for Dispatch
                      </button>
                      <button onClick={() => runProduction(c)} disabled={processingId === c.id} className="bg-purple-600 hover:bg-purple-500 text-white font-black py-2.5 px-4 rounded-xl text-xs flex items-center gap-1 disabled:opacity-50">
                        <Factory size={14} /> Route to Production
                      </button>
    ...
    <div className="space-y-4 md:space-y-8 pb-20 px-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="theme-text-secondary text-sm font-medium uppercase tracking-widest">Store — Returns & Replacements</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          <Section title="Returns — Awaiting Store" icon={<Box size={18} className="text-red-400" />} color="bg-red-500/20" list={returns} kind="return" emptyText="No pending returns. Returned orders from Inventory View will appear here." />
          <Section title="Replacements — Approved by Faisal, Awaiting Store" icon={<Package size={18} className="text-blue-400" />} color="bg-blue-500/20" list={replacements} kind="replacement" emptyText="No approved replacements. Faisal-approved replacements will appear here." />
        </div>
      )}
    </div>
  );
