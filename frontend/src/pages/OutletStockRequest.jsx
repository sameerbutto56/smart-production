    ...
    <div className="space-y-8 pb-20 px-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-white tracking-tight">{outletName}</h1>
            <p className="text-gray-400 text-sm font-medium uppercase tracking-widest">Outlet Stock Request Portal</p>
          </div>
        </div>
      ...
      <div className="flex bg-gray-950 border-2 border-gray-900 rounded-2xl p-1.5 overflow-x-auto no-scrollbar">
        <button onClick={() => setActiveTab('request')}
          className={`px-6 py-3 text-xs font-black rounded-xl transition-all whitespace-nowrap uppercase tracking-wider flex items-center space-x-2 ${
            activeTab === 'request' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-gray-800'
          }`}>
        ...
        <button onClick={() => setActiveTab('status')}
          className={`px-6 py-3 text-xs font-black rounded-xl transition-all whitespace-nowrap uppercase tracking-wider flex items-center space-x-2 ${
            activeTab === 'status' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-gray-800'
          }`}>
        ...
        <>
          ...
          {activeTab === 'request' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="relative group">
                  ...
                  <input type="text" placeholder="Search items to request..." value={searchTerm}
                    ...
                    className="w-full bg-gray-950 border-2 border-gray-900 rounded-2xl py-4 pl-12 pr-6 focus:outline-none focus:border-blue-500 transition-all font-medium text-gray-300"
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setSearchTerm('')}
                    className={`px-3 py-1.5 text-[9px] font-black uppercase tracking-wider rounded-xl border transition-all ${
                      !searchTerm ? 'bg-blue-600 text-white border-blue-500' : 'bg-gray-900 text-gray-500 border-gray-800 hover:text-white'
                    }`}>All</button>
                  {categories.map(cat => (
                    <button key={cat} onClick={() => setSearchTerm(cat)}
                      className={`px-3 py-1.5 text-[9px] font-black uppercase tracking-wider rounded-xl border transition-all ${
                        searchTerm === cat ? 'bg-blue-600 text-white border-blue-500' : 'bg-gray-900 text-gray-500 border-gray-800 hover:text-white'
                      }`}>{cat}</button>
                  ))}
                </div>
                  ...
                  {filteredInventory.map((item, i) => {
                    ...
                    return (
                      <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                        className="glass p-5 rounded-2xl border-2 border-gray-900 hover:border-blue-500/30 transition-all">
                        ...
                        <div className="flex items-center justify-between mt-4">
                          <p className="text-lg font-bold text-gray-400">{item.color ? [item.color, item.size].filter(Boolean).join(' • ') : item.size || item.fabric || ''}</p>
              ...
              {/* Cart Sidebar */}
              <div className="glass p-6 rounded-2xl border-2 border-gray-900 lg:sticky lg:top-6 h-fit">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="font-black text-white uppercase tracking-wider text-sm flex items-center space-x-2">
                ...
                {cartItems.length === 0 ? (
                  <div className="text-center py-10">
                    ...
                    <p className="text-[10px] text-gray-600 font-bold mt-1">Add items to request stock</p>
                  </div>
                ) : (
                  <div className="space-y-3 mb-6">
                    {cartItems.map(item => (
                      <div key={item.id} className="flex items-center justify-between p-3 bg-gray-800/30 rounded-xl border border-gray-800">
                        <div className="flex-1">
                          <p className="font-bold text-white text-xs">{item.name}</p>
                          <p className="text-[9px] text-gray-500 font-bold uppercase">{item.category}</p>
                        </div>
                        <div className="flex items-center space-x-2">
                ...
                {cartItems.length > 0 && (
                  <>
                    <div className="flex justify-between items-center py-3 border-t border-gray-800 mb-4">
                      <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Total Items</span>
          ...
          {activeTab === 'status' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="glass p-4 rounded-2xl border-2 border-gray-900">
                ...
                <div className="glass p-4 rounded-2xl border-2 border-gray-900">
                  ...
                  <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Approved</p>
                </div>
                <div className="glass p-4 rounded-2xl border-2 border-gray-900">
                  <p className="text-2xl font-black text-red-400">{statusCounts.rejected}</p>
                  <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Rejected</p>
                </div>
                <div className="glass p-4 rounded-2xl border-2 border-gray-900">
              ...
              <div className="space-y-3">
                {requests.length === 0 ? (
                  <div className="text-center py-16">
                    ...
                    <p className="text-gray-500 font-black text-xs uppercase tracking-widest">No requests yet</p>
                    <p className="text-[10px] text-gray-600 font-bold mt-1">Go to Request Stock tab to place an order</p>
                  </div>
                ) : (
                  requests.map((req, i) => {
                    ...
                    return (
                      <motion.div key={req.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                        className="glass p-5 rounded-2xl border-2 border-gray-900 hover:border-gray-800 transition-all">
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <div>
                              ...
                              <div className="flex items-center space-x-4 mt-1">
                                <p className="text-[10px] font-bold text-gray-500">Requested: {req.quantity}</p>
                                <p className="text-[10px] font-bold text-emerald-500">Approved: {req.approvedQty}</p>
                                <p className="text-[10px] font-bold text-gray-600">{new Date(req.createdAt).toLocaleDateString()}</p>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center space-x-4">
                            <div className="text-right">
                              <p className="text-lg font-black text-white">{req.quantity - req.approvedQty}</p>
                              <p className="text-[9px] text-gray-600 font-bold uppercase">Pending</p>
                            </div>
                          </div>
                        </div>
                        {req.notes && (
                          <div className="mt-3 p-3 bg-gray-800/30 rounded-xl border border-gray-800">
                            <p className="text-[9px] font-black text-gray-500 uppercase tracking-wider">Store Notes</p>
                            <p className="text-sm text-gray-300 font-medium mt-1">{req.notes}</p>
                          </div>
                        )}
                      </motion.div>
                    );
