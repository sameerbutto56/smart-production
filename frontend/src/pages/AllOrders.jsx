  ...
  return (
    <div className="space-y-6 md:space-y-10 max-w-7xl mx-auto pb-20 px-4">
      ...
      {showModal && selectedOrder && (() => {
        ...
        const isOutletOrder = selectedOrder.source === 'OUTLET';
        const outletItems = isMultiItem ? allItems : [{ productDetails: product, quantity: selectedOrder.quantity }];
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-10 custom-scrollbar theme-text-primary">
                ...
                <>
                  ...
                  {(() => {
                    const selectedSize = selectedOrder.size || '';
                    ...
                    return (
                      <section>
                        ...
                        {perProductEntries.length > 0 ? (
                          ...
                          <div className="theme-bg p-4 md:p-6 rounded-3xl border theme-border grid grid-cols-2 md:grid-cols-4 gap-2">
                  ...
                  {(() => {
                    ...
                    const hasEng = hasEngravingData(custom) || (Array.isArray(en) && en.length > 0) || (Array.isArray(el) && el.length > 0) || selectedOrder.engravingText || selectedOrder.engravingInstructions || selectedOrder.logoRequired || selectedOrder.logoDesign || selectedOrder.instructionNotes;
                            ...
                            return (
                              <div key={idx} className="bg-purple-600/5 p-4 md:p-6 rounded-2xl border border-purple-500/10">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  {(selectedOrder.engravingType || custom?.engravingType) && (
                                    <div className="bg-gray-800/60 p-3 rounded-xl">
                                      ...
                                      <p className="text-xs font-black text-violet-400">{(selectedOrder.engravingType || custom?.engravingType) === 'direct' ? 'Direct Engraving' : 'Patch Engraving'}</p>
                                    </div>
                                  )}
                                  {nameLines.length > 0 && (
                                  ...
                                  {(custom?.nameColor || custom?.logoPlacement || custom?.logoColor) && (
                                    <div className="space-y-2">
                                      <div className="flex flex-wrap gap-1">
                                        ...
                                        {custom?.logoPlacement && <span className="text-[9px] font-black text-teal-400 bg-teal-900/30 px-1.5 py-0.5 rounded">Pos: {fmtPos(custom.logoPlacement)}</span>}
                                ...
                                {(custom?.designNotes || selectedOrder.engravingText || selectedOrder.engravingInstructions) && (
                                  <div className="mt-3 bg-yellow-500/5 p-3 rounded-xl border border-yellow-500/10">
                                    ...
                                    <p className="text-xs font-bold text-yellow-300/90 italic leading-tight">{isUrdu ? romanToUrdu(custom?.designNotes || selectedOrder.engravingText || selectedOrder.engravingInstructions) : (custom?.designNotes || selectedOrder.engravingText || selectedOrder.engravingInstructions)}</p>
                                  </div>
                                )}
                              </div>
                            );
                  ...
                  {(() => {
                    ...
                    const orderNotes = [selectedOrder.instructionNotes, selectedOrder.engravingInstructions].filter(Boolean);
