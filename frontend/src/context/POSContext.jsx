...
export function POSProvider({ children }) {
  ...
  const downloadExcel = useCallback(async () => {
    try {
      ...
      // Export using standardized 4-tier simplified financial summary
      exportInvoicesToExcel({
        sales: srcSales,
        returns: srcReturns,
        balancePayments: srcBalancePayments,
        journalEntries,
        summary,
        outlet: selectedOutlet,
        rangeLabel: salesRange === 'custom' ? `${salesDateFrom} to ${salesDateTo}` : (salesRange || 'all'),
      });
